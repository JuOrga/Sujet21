// LE JOURNAL DES LIVRAISONS — les données, et rien d'autre.
//
// Ce fichier est LOURD (un demi-mégaoctet de prose) et c'est normal : c'est
// le journal du chantier. Ce qui n'était pas normal, c'est qu'il parte dans
// le paquet de TOUS les joueurs — mesuré : 484 Ko bruts, 187 Ko une fois
// comprimés, soit un tiers du téléchargement, pour un écran que seul le
// concepteur ouvre. Les chaînes ne se minifient pas : le poids du texte est
// le poids du texte.
//
// Il n'est donc plus JAMAIS importé statiquement. changelog.ts le charge à
// la demande (import dynamique) quand l'écran NOTES DE VERSION s'ouvre, et
// Vite lui donne son propre morceau. Importer ce fichier en direct depuis
// le jeu le ferait retomber dans le paquet principal — un test le garde.
//
...
export const DELIVERIES: Delivery[] = [
  {
    date: '15/09/2026 00:21',
    title: 'LA GRILLE DE REPÉRAGE DES BOÎTES : chaque pixel ne regarde plus que les boîtes de sa case',
    notes: [
      'LA DEMANDE : « sur iPad c’est cata aussi sur démineur » — avec le rapport de performance (iPad Pro M1, Chrome iOS, résolution faible, 1,26 mégapixel) : 14 im/s, 466 images sur 466 au-dessus de 33 ms. Par image : 14 ms de physique, 0,3 ms de laser, 1,2 ms d’autre JavaScript… et 56 ms HORS processeur. Le processeur avait fini en 15 ms ; le reste, c’était la carte graphique. Les durées d’image se groupaient à 52 et 73 ms, trois à cinq périodes d’écran : un GPU qui ne finit pas son dessin à temps.',
      'POURQUOI DÉMINEUR ET PAS LES AUTRES. L’A/B du 30/08 sur ce même iPad avait mesuré 17,7 ms par mégapixel sur une salle ordinaire (45 im/s en faible). Ici, même résolution, 56 ms : trois fois plus. La différence est le tableau : 112 boîtes, le plafond du budget. Le shader de composition parcourait TOUTES les boîtes pour CHAQUE pixel, trois fois (la salle des coques, la couverture, puis le corps de chaque boîte — relief, ombre, aura, biseau). 1,26 mégapixel × 112 × 3, dont l’immense majorité pour des boîtes à l’autre bout du tableau. Le coût est proportionnel aux pixels multipliés par les boîtes, et démineur maximise le second facteur — c’est aussi, sans doute, une part des 13 im/s de la GTX 1060 sur Firefox.',
      'CE QUI CHANGE (render/grilleBoites.ts, renderer.ts). Le tableau est découpé en cases (128 au plus, à peu près carrées), et pour chaque case on dresse d’avance la liste des boîtes qui peuvent teinter un de ses pixels. Elle part au shader en UNIFORMES — un ivec4 par case : l’octet 0 le compte, puis quinze indices dans l’ordre du tableau — parce que la composition occupe déjà les seize unités de texture garanties par WebGL2. Les trois boucles sur les boîtes ne parcourent plus que la liste de la case du pixel. Sur un damier de 112 boîtes comme démineur : 14 × 9 cases, 4,8 boîtes par case en moyenne, 7 au plus — au lieu de 112. La grille se reconstruit à chaque image en 0,03 ms (mesuré au banc, Node).',
      'LA MARGE, ce qui rend le résultat identique au pixel près. Une boîte teinte au-delà de son contour : l’ombre portée jusqu’à 56 u, l’aura selon le matériau (hydro, froid, chaud × aura propre), le liseré et les fondus en N / zoom, le décalage du relief. Chaque boîte entre donc dans la liste de toutes les cases que son rectangle englobant (rotation comprise) GONFLÉ de cette marge touche — la marge se calcule pour l’image depuis les mêmes valeurs que le shader (margeInfluence). Le sas, le vide et la baie, que le shader ne court-circuite jamais, vont dans toutes les cases. Une case qui déborderait (plus de quinze boîtes) dit « toutes » : le shader y fait comme avant — aucune boîte oubliée, jamais.',
      'L’A/B EN PRÉVISUALISATION : ?grille=0 débranche la grille (chaque case dit « toutes »). Même adresse, deux rapports, et le rapport dit désormais l’état de la grille (active, cases, débordements). C’est le chiffre hors processeur qui donnera le gain réel sur l’iPad et sur la 1060.',
      'VÉRIFIÉ : onze tests neufs (render/grilleBoites.spec.ts) — la force brute d’abord : douze tirages de 40 à 140 boîtes au hasard (pivotées, « partout ») et 1 500 points chacun, y compris hors de l’emprise ; pour chaque point, chaque boîte à portée doit être dans la liste de sa case. Puis l’ordre du tableau dans chaque liste, le débordement qui dit « toutes », les boîtes « partout » dans chaque case, le rectangle d’une boîte pivotée, le damier de 112 sans débordement, tous les tableaux du dépôt sans débordement à 130 u de marge, la marge d’influence, et le CONTRAT AVEC LE SHADER : le test lit la source de la composition et refuse toute portée fixe (56, 60, 34…) au-dessus de PORTEE_FIXE et tout « N / uZoom » au-dessus de PORTEE_PAR_ZOOM. 1225 verts dans 115 fichiers, type-check à 0, build propre. Rendu rejoué dans Chromium sans tête : les sept programmes compilent avec les nouveaux uniformes, l’écran de chargement s’efface, aucune erreur.',
    ],
  },
  {
    ...
    notes: [
      'LA DEMANDE : « sur Chrome j’ai ce message après quelques secondes de chargement puis si je refresh ça bloque le navigateur pendant le chargement du jeu. Mon ami lui a le message plusieurs fois de suite » — le panneau LE PROTOCOLE MET TROP LONGTEMPS À S’OUVRIR, dernière étape franchie : le rendu se compile.',
      'D’OÙ ÇA VIENT. Sur Chrome, le rendu se compile en coulisse (KHR_parallel_shader_compile) : le fil principal reste libre, la boucle tourne et demande au pilote, à chaque image, s’il a fini. Mais l’étape « rendu » n’était dite qu’UNE fois à la garde d’amorçage, et plus rien ensuite — de son point de vue, douze secondes sans signe de vie, et elle sonnait. Sur Windows, ce pilote c’est le compilateur Direct3D (fxc, derrière ANGLE), et sur un shader de composition de cent kilo-octets il dépasse aisément les douze secondes à la première visite — puis à chaque livraison, puisque la source change et que le cache disque de Chrome ne la reconnaît plus (trois livraisons ce jour-là). Le panneau conseillait alors de RECHARGER : la compilation repartait de zéro, le panneau revenait douze secondes plus tard — « plusieurs fois de suite » — et, entre les deux, le navigateur se figeait : le processus graphique de Chrome attend que les fils de compilation de l’ancien contexte aient fini avant de le lâcher, et rien ne se dessine pendant ce temps, pas même l’interface du navigateur.',
      'CE QUI CHANGE. Tant que le rendu compile, main.ts envoie un POULS à la garde à chaque image (__sujet21Compile), avec l’avancement : programmes liés / programmes en tout — Programmes (render/programmes.ts) interroge désormais les sept programmes à chaque image et compte les finis, au lieu de s’arrêter au premier qui travaille encore. L’écran de chargement l’affiche : « le rendu se compile… 3/7 », puis, passé douze secondes, « la carte graphique est lente, on patiente (ne rechargez pas) ». La garde, elle, ne sonne plus tant que le pouls bat : sa prochaine sonnerie tombe douze secondes après le DERNIER pouls (un fil qui se fige reste signalé à l’heure), et sa patience a une borne — deux minutes, le pire mesuré (Firefox sur Windows, 14/09). Passé ça, un panneau nouveau, LE RENDU MET TROP LONGTEMPS À SE COMPILER, dit ce qu’il sait (depuis combien de secondes, combien de programmes liés), que le jeu est vivant, et surtout qu’il ne faut PAS recharger ; la première image le retire, comme avant.',
      'CE QUI NE CHANGE PAS : sans compilation parallèle (Firefox), le verdict bloque toujours le fil une image après le lancement — pas de pouls pendant le gel, et si le gel dépasse douze secondes la sonnerie tombe au dégel, juste avant la première image qui la retire (l’éclair d’avant, avec le nouveau texte). Et un fichier qui n’arrive pas garde son propre délai (quatre-vingt-dix secondes).',
      'VÉRIFIÉ : trois tests neufs sur la garde (amorce-garde.spec.ts : le pouls fait patienter et montre l’avancement ; un pouls qui s’arrête sonne douze secondes après ; deux minutes de pouls sans image sonnent, et la première image retire le panneau) et un sur Programmes (programmes.spec.ts : le compte avance programme par programme) — rouges sans le correctif (3 sur 9, 1 sur 5), verts avec. 1214 verts dans 114 fichiers, type-check à 0, build propre. Ce qu’on ne peut pas mesurer d’ici : le temps de compilation sur le Chrome en question — le rapport de PARAMÈTRES (compileRenduMs, compileParallele) le dira, et dira aussi si le cache disque de Chrome reprend la main à la seconde visite.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : « et les deux minutes de page noire j’aimerais régler ça » — le rapport de performance de la salle démineur (Firefox 155 sur Windows) donnait 118,6 s entre la naissance du rendu et la première image (compileRenduMs), sans compilation parallèle (compileParallele : false). Deux minutes de page noire, à chaque chargement, le fil principal gelé sur « le rendu se compile… ».',
      'D’OÙ ÇA VIENT, EN DEUX TEMPS. Firefox n’expose pas KHR_parallel_shader_compile (bug Mozilla 1736076, la fiche MDN se trompe) : la question bloquante LINK_STATUS lui est posée une image après le lancement, et le fil principal reste gelé le temps que le pilote finisse. Et ce temps, sur Windows, est celui du compilateur Direct3D (fxc, derrière ANGLE) — qui ne tolère pas une opération de GRADIENT (un texture() sans niveau de détail explicite, un dFdx/dFdy) dans une boucle à flot divergent (break, continue) : il DÉROULE alors la boucle entière. La boucle des boîtes de la composition en avait quatre — l’atlas des habillages, l’iris du sas, le gradient du SDF pour la tranche du relief et pour le biseau d’éclairage — pour une borne constante de MAX_BOXES = 160 et un corps de cinq cents lignes où formeSdf s’inline trois fois. Cent soixante copies d’un corps énorme : fxc y passe ses deux minutes. Chrome subit le même compilateur mais compile dans ses fils et garde un cache disque — le prix s’y paie une fois, en coulisse ; Firefox le paie à chaque visite, devant.',
      'CE QUI CHANGE (render/renderer.ts) : plus aucune opération de gradient dans les boucles sur les boîtes. Les deux échantillonnages passent par textureGrad avec la dérivée écrite explicitement (pxMonde, la largeur d’un pixel en unités monde, divisée par l’échelle de chaque texture) ; le gradient du SDF vient de différences finies dans le repère local de la boîte (gradSdfBoite) — ses deux usages n’en prenaient que la direction, et la différence finie sur un champ de distance vaut la dérivée d’écran à un facteur positif près. Mieux, même : dFdx lisait la valeur du pixel voisin, indéfinie quand ce voisin avait sauté la boîte d’un continue. Et la borne des six boucles sur les boîtes (trois par shader) devient min(uBoxCount, MAX_BOXES) : un nombre que le compilateur ne connaît pas ne se déroule pas.',
      'CE QUI NE CHANGE PAS : le résultat à l’écran, à un détail près qu’on assume — le niveau de mipmap des habillages et de l’iris sur une boîte PIVOTÉE se calcule sans la rotation (même longueur de dérivée, axes non tournés), imperceptible sur des motifs sans couture. MAX_BOXES reste défini dans les deux shaders (budgets.spec.ts le garde) : c’est la taille des tableaux d’uniformes.',
      'CE QU’ON NE PEUT PAS MESURER D’ICI : fxc ne tourne que sur Windows. Le rapport de PARAMÈTRES le dira — compileRenduMs, sur le même Firefox, avant et après. Si le chiffre ne bouge pas, la piste est fausse et on le saura sans débat.',
      'VÉRIFIÉ : un test neuf (render/gradientBoucles.spec.ts) lit la source des deux shaders, suit le graphe d’appel de chaque boucle sur les boîtes, et refuse un texture() implicite, un dFdx/dFdy ou une borne constante — sur la source d’avant, trois de ses quatre vérifications tombent (rouge) ; avec le correctif, vert. 1191 verts dans 113 fichiers, type-check à 0, build propre. Les sept programmes compilés et liés dans un WebGL2 réel (Chromium sans tête, SwiftShader) : l’écran de chargement s’efface à la première image, aucune erreur de page, aucun panneau de panne. À juger en prévisualisation : les habillages texturés, l’iris du sas, la tranche du relief et le biseau d’éclairage sur des blocs droits et pivotés.',
    ],
  },
  {
    date: '14/09/2026 14:59',
    title: 'LA SALLE DÉMINEUR À 10 IM/S : le traceur laser testait toutes les boîtes du tableau à chaque pas',
    notes: [
      'LA DEMANDE : « sur la salle démineur mon ami a de très mauvaises performances » — avec le rapport de performance (Firefox 155 sur Windows, 8 cœurs, 1768 × 867, résolution moyenne) : 10 im/s en médiane, 2 789 images sur 3 922 au-delà de 50 ms, et dans cette bande 97 ms d’« autre JS » par image contre 12,7 ms de physique et 0,4 ms de rendu. Ni la cuve ni le GPU : notre code, hors des deux gros postes — et le rapport ne disait pas lequel.',
      'D’OÙ ÇA VIENT : le tableau porte 112 boîtes, 7 émetteurs laser et 7 cibles. À CHAQUE pas de 5 unités, chaque faisceau testait TOUTES les boîtes du tableau, deux fois (le miroir fixe, puis la paroi), plus les portes fermées — et demandait trois fois les voisins du même point à la simulation (glace au contact ? eau liquide ? vapeur ?). Sept faisceaux qui traversent un tableau de 2 800 u : ~4 000 pas × 230 tests de rectangle par image. Le coût grimpait avec le NOMBRE de boîtes, pas avec ce que le faisceau rencontrait — et le budget de blocs venait de monter.',
      'CE QUI CHANGE, DANS LE TRACEUR (game/laser.ts) : au départ de chaque segment droit (émetteur, rebond, dioptre, rail), on calcule pour chaque boîte l’intervalle de distance pendant lequel le rayon peut se trouver dans son enveloppe (test des « slabs » sur la boîte englobante alignée — rotation comprise —, gonflée d’un pas et demi) ; un pas ne teste plus au contact près que les boîtes dont l’intervalle contient sa distance parcourue. Les boîtes transparentes (grille, sas, vide, baie) ne sont même plus dans la liste. Le résultat est le même au pas près : le miroir garde la main sur la paroi qui le chevauche, dans l’ordre du tableau, comme avant.',
      'CE QUI CHANGE, DANS LA SIMULATION (sim/solver.ts, milieuAt) : les trois questions du pas — glace, eau, vapeur — en UN parcours de voisins au plus grand des deux rayons, avec exactement les règles de iceNormalAt, liquidAt et gasAt (un test le garantit point par point sur un peigne qui traverse les trois corps). Le traceur ne calcule plus la normale de glace, coûteuse, qu’où la glace est là. L’éditeur et les tests, qui n’ont pas ce raccourci, tracent comme avant.',
      'CE QUE ÇA RAPPORTE, MESURÉ AU BANC (Node, V8 — pas la machine de l’ami) sur un damier de 112 boîtes de 2 800 u et sept faisceaux qui le traversent : cuve vide 12,1 → 0,36 ms par image ; 900 gouttes hors du trajet 14,0 → 0,72 ms ; 900 gouttes sur le trajet 14,4 → 4,0 ms ; 900 gouttes gelées sur le trajet 13,1 → 3,8 ms. Sur Firefox, où le même code coûtait 97 ms, le gain reste à mesurer sur place — le rapport le dira.',
      'LE RAPPORT SAIT MAINTENANT LE DIRE : version 5 — le traçage des faisceaux est chronométré à part dans la boucle d’image et paraît comme « laserMs » dans les moyennes de la fenêtre, dans chaque bande de lenteur et dans les pires images. L’« autre JS » n’est plus un sac : si la salle démineur rame encore, on saura si c’est le laser ou autre chose.',
      'CE QUE LE RAPPORT DIT AUSSI, NON TRAITÉ ICI : 118,6 s entre la naissance du rendu et la première image (compileRenduMs), sans compilation parallèle des shaders (compileParallele : false). Deux minutes de page noire au chargement sur ce Firefox Windows — à regarder à part, la cadence en jeu n’en dépend pas.',
      'VÉRIFIÉ : sept tests neufs dans game/laser.spec.ts — l’intervalle du rayon (devant, dedans, à côté, derrière, vertical, diagonale), l’enveloppe d’une boîte pivotée, une barre pivotée qui absorbe là où sa boîte min/max ne passe pas, le budget (un tableau de 112 boîtes ne coûte pas 112 tests par pas : 44 576 tests sans le filtre, rouge ; moins de 4 par pas avec, vert), le miroir qui garde la main sur la paroi, le milieu en un passage contre les trois requêtes, et le tracé identique avec ou sans le raccourci dans un corps simulé. 1165 verts dans 110 fichiers, type-check à 0, build propre.',
    ],
  },
  {
  ...
  {
    date: '12/09/2026 13:04',
    title: 'LE PREMIER CHARGEMENT : un écran de chargement dès le premier pixel, le rendu compilé en coulisse, et la veille qui ne recouvre plus un jeu qui tourne',
    notes: [
      'LA DEMANDE : « le chargement pour la première fois, dès qu’on accède à l’URL, ne convient pas : cela bloque le navigateur pendant de longues secondes, aucun indicateur de chargement, et parfois ce message » — le panneau LE PROTOCOLE MET TROP LONGTEMPS À S’OUVRIR, alors que le jeu tournait derrière.',
      'LA CAUSE, à la lecture : le shader de composition pèse 107 ko de GLSL (deux mille lignes, vingt-deux boucles), et le Renderer le compilait, le liait et lui demandait son verdict (LINK_STATUS) et ses uniformes À SA NAISSANCE, en plein milieu de l’évaluation du module — le pilote devait finir tout le travail sur-le-champ, dans le fil principal, et le navigateur ne peignait rien tant que ce n’était pas fait : ni la fiche, ni un mot. La veille de douze secondes, elle, partait à l’analyse du document et ne savait rien de ce gel : quand il durait plus qu’elle, elle sonnait au dégel, la première image arrivait juste après, et rien ne retirait son panneau.',
      'LE RENDU EN COULISSE (render/programmes.ts) : compilation et édition de liens sont LANCÉES sans rien attendre ; avec KHR_parallel_shader_compile (Chrome, Edge, Firefox), le pilote travaille dans ses fils et la boucle de rendu ne pose la question bloquante qu’une fois COMPLETION_STATUS_KHR levé pour les six programmes — elle ne simule ni ne dessine avant, la page reste vivante. Sans l’extension, la question bloque comme avant, mais UNE image plus tard : l’écran a eu le temps de peindre le mot du chargement. Un shader refusé se dit au verdict, avec le journal du fautif, comme il se disait au constructeur.',
      'L’ÉCRAN DE CHARGEMENT : un voile sur toute la fenêtre, porté par le document lui-même (visible sans JavaScript, couleurs littérales comme le panneau de panne) — le titre, une barre qui balaie, et l’étape en cours : « le fichier arrive… », « le module s’ouvre… », « le rendu se compile… ». Il voile la fiche tant que le jeu n’est pas prêt derrière (un COMMENCER qu’on voit sans pouvoir le presser mentirait) et part en fondu à la première image. main.ts lui raconte chaque étape (__sujet21Etape). Un premier jet en faisait un simple cartouche en bas de l’écran ; la réponse a préféré l’écran plein.',
      'LA VEILLE, RÉÉCRITE : douze secondes SANS SIGNE DE VIE, et chaque étape franchie la réarme — un démarrage lent mais vivant n’est plus une panne. Si elle a sonné trop tôt, la première image retire son panneau ; une vraie erreur (exception, promesse rejetée, WebGL2 absent) reste affichée. Et un fichier qui n’est toujours pas arrivé n’est pas une panne : le cartouche dit « la connexion est lente, on patiente », jusqu’à quatre-vingt-dix secondes.',
      'LES TEXTURES, DÉCODÉES HORS DU FIL PRINCIPAL : texImage2D depuis un <img> décode et retourne l’image sur place — 64 Mo de pixels pour le ciel (4096 × 4096), 32 pour l’atlas des parois, plus une vingtaine d’autres, chacune au moment où elle arrive, pendant que le joueur lit la fiche. createImageBitmap fait le décodage dans un fil du navigateur ; il ne reste que le retournement (FLIP_Y, le même partout) et la copie. Même alpha non prémultiplié : le rendu est le même au pixel près.',
      'LE RAPPORT DE PERFORMANCE dit désormais ce que l’amorçage a coûté sur LA MACHINE DU JOUEUR (amorcage : compileRenduMs, premiereImageMs, compileParallele) : la durée de la compilation sur une carte ordinaire est le poste qu’on soupçonne sans avoir pu le mesurer d’ici.',
      'VÉRIFIÉ : dix tests — quatre sur la compilation en coulisse (jamais la question bloquante avant « fini », une image de grâce sans l’extension, le journal du shader fautif, les programmes par nom) et six sur la garde, rejouée telle quelle depuis index.html sur un document de carton (la veille sonne, une étape la réarme, la première image retire son panneau et le mot, une vraie erreur reste, la connexion lente patiente, sans WebGL2 rien ne s’arme). Dans Chromium sans tête (SwiftShader, sans l’extension, textures écartées) : le module s’ouvre à 143 ms, le rendu attend à 442 ms, le mot s’affiche puis s’efface à la première image, aucun panneau. Ce Chromium compile le GLSL en 44 ms et dessine chaque image en vingt secondes de logiciel : ni la durée de compilation ni le coût des textures ne s’y mesurent — c’est le rapport de PARAMÈTRES qui le dira sur un vrai PC. 1 115 tests verts, type-check à zéro, construction passée.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      ...
      'COPIER, COUPER, COLLER : Ctrl+C copie la sélection (une ou plusieurs), Ctrl+X la copie et la supprime, Ctrl+V colle — le CENTRE du groupe se pose SOUS LA SOURIS, aimanté à la grille (sans souris sur la carte : un pas vers l’est, comme Dupliquer), et les nouveaux venus deviennent LA sélection : on les prend aussitôt et on les glisse. Ctrl+V encore : un autre groupe, indépendant. Ctrl+Z défait un collage d’un coup. Le panneau gagne les boutons Copier et Coller (au doigt, sans clavier, c’est le chemin — le collage tombe alors d’un pas vers l’est) ; le stylet est suivi comme la souris. Sans sélection d’élément, ou avec du texte sélectionné dans la page, Ctrl+C reste au navigateur ; une touche tenue ne se répète pas. Ctrl+X et « Tout supprimer » retirent le groupe en UN pas d’annulation (avant, un par élément).',
      'LE PRESSE-PAPIER (editor/pressePapier.ts, pur et testé) tient des CLONES, pas des indices : supprimer entre la copie et le collage ne le fausse pas. Une pastille copiée porte son numéro logique en clair (une « cible 1 » collée reste une cible 1, comme au Dupliquer), une étiquette copiée perd sa clé (elle désigne UN panneau que le jeu manipule), les lampes au-delà du plafond de quatre sont refusées et comptées, les uniques sont nommés dans la barre d’état plutôt que copiés. Le cadre juge une pièce TOURNÉE sur ce qu’on voit (la boîte de ses coins pivotés), pas sur son repère local : une paroi de 200 × 20 à 45° couvre un carré de 155, et c’est ce carré qu’il faut entourer.',
      'VÉRIFIÉ : treize tests sur le cadre (entièrement dedans, effleuré dehors, la pièce tournée, tracé dans n’importe quel sens), la copie (clones, emprise, uniques nommés, numéro de pastille, clé d’étiquette, doublons, ordre de peinture), le collage (décalage, références des nouveaux venus, originaux intacts, deux collages indépendants, plafond des lampes) et le décalage sous le curseur. 1105 tests verts dans 102 fichiers, type-check à 0, build propre. Le jeu ne se monte pas dans le Chromium sans tête de l’environnement : le cadre n’a pas été tracé à la souris ici — à voir sur l’aperçu (previsu-go).',
    ],
  },
  {
    ...
    notes: [
      'LA DEMANDE : le tableau « démineur » d’un concepteur est fait d’UNE cellule — une chambre chanfreinée, sa cachette voilée, sa pastille numérotée, ses portes asservies à cheval sur la paroi — et il veut la reproduire facilement, « quatre par tableau par exemple », en gérant les JOINTURES. Dupliquer (D) reproduisait un élément à la fois et laissait à la main tout ce qui fait la jointure entre deux cellules.',
      'LE GESTE : sélectionner la chambre (le motif est alors la coque et tout ce qui est centré dans son emprise — les portes posées sur sa paroi en font partie), ou une sélection multiple (prise telle quelle) ; dans le panneau, un bloc PAVAGE : colonnes (vers l’est), rangées (vers le sud), la jointure, la numérotation des canaux, et PAVER. Ctrl+Z défait tout d’un coup. Le départ, le sas, la fiole, le banc et le marchand, uniques, restent où ils sont et sont nommés.',
      'LES JOINTURES (editor/pavage.ts, pur et testé). Paroi commune : les coques se recouvrent d’une épaisseur (celle dessinée, par pas de 8), la paroi mitoyenne est UNE paroi. Deux coques qui ne font que se toucher ne devinent aucune porte (le milieu d’une face n’entre jamais dans le vide de l’autre) : les faces en regard sont donc FORCÉES ouvertes des deux côtés, à la largeur de la porte qui traverse la jointure, arrondie PAR DÉFAUT au pas du format (200 → 192) pour que la fente ne dépasse jamais la porte qui la couvre ; sans porte, à la porte du kit (256). La porte de chaque jointure est RECONSTRUITE : elle traverse d’une face intérieure à l’autre (la paroi partagée, ou les deux parois accolées), centrée sur la face — la fente d’une coque est toujours au milieu. Un motif percé à l’est ET à l’ouest ne double pas la porte : la première cellule l’emporte. Les portes du motif qui tombent sur le BORD du pavage et que rien n’attend dehors (ni cellule voisine, ni coque étrangère) sont retirées, et la face refermée ; celles qu’un couloir attend restent.',
      'LES CANAUX : chaque cellule reçoit les numéros LIBRES suivants (une pastille étrangère au tableau qui porte déjà le 3 est sautée), la copie porte son numéro en clair, et ses portes et chasses suivent — la porte d’une jointure appartient à la cellule qui l’apporte. Décoché : les copies partagent les canaux du motif. Les autres jointures : ACCOLÉE (deux parois dos à dos, la porte traverse les deux), ESPACÉE (un écart, rien n’est percé, le motif est recopié tel quel — à relier soi-même par des couloirs). La cuve s’agrandit si le pavage déborde, et la barre d’état dit tout : cellules, éléments, portes à cheval, portes de bord retirées, lampes refusées au plafond.',
      'VÉRIFIÉ : dix-neuf tests sur la cellule du démineur (le motif d’une chambre, le pas, les faces forcées et refermées, la jointure VRAIMENT percée sur la coque expansée — libre au centre de la porte, pleine à côté —, les portes reconstruites et recentrées, les canaux libres et communs, la cuve agrandie, le doublon est/ouest, la porte qu’un couloir attend, la coque tournée, l’accolée, l’espacée, la phrase du bilan). 1092 tests verts dans 101 fichiers, type-check à 0, build propre. Le jeu ne se monte pas dans le Chromium sans tête de l’environnement : le bloc du panneau n’a pas été cliqué dans un navigateur ici — à voir sur l’aperçu (previsu-go).',
    ],
  },
  {
    ...
    notes: [
      'LA DEMANDE : sur l’esquisse docs/sujet-vivant.md (le diagnostic et les pistes pour que le volume soit un être), le concepteur retient les motes (G1) et le halo (G5), et trouve le dessin des gouttes éjectées « pas terrible » — sans toucher aux présets. Tout ce qui suit est du RENDU : aucun paramètre de simulation, aucun préset du banc n’a bougé ; le banc n’envoyant pas de présence, son halo vaut zéro et ses points de vie sont ceux du jeu — rien de simulé en plus.',
      'LES MOTES (render/vie.ts) : quarante-quatre grains en suspension dans le corps, chacun ACCROCHÉ à une goutte hôte du corps qu’il suit — l’advection par le fluide vient gratuitement, et une mote ne quitte le corps qu’en étant éjectée avec sa goutte : elle se raccroche alors ailleurs, en fondu. Elles frétillent d’un petit cercle autour de leur hôte, changent d’hôte d’elles-mêmes toutes les six à seize secondes (elles circulent), se FIGENT dans la glace (pâlies), s’ÉLARGISSENT dans la vapeur, et sous la peur (réserve à sec) se RESSERRENT vers le regard, comme un banc qui se serre ; corps défait, elles dérivent et s’éteignent une à une. Dessinées dans une passe additive après la composition, masquées par le champ : une mote n’existe que DANS le corps principal, et se voile derrière la peau vers le bord.',
      'LE HALO (renderer, uHalo) : le corps est une SOURCE — une gaussienne cyan discrète au sol autour du centroïde, large comme le corps et demi, qui pulse AU MÊME RYTHME que la respiration du contour. Plein au calme, il se rétracte sous la peur (0,45 — il se cache), baisse un peu en visée, s’éteint quand le corps se défait. Versé avant l’eau : sous le corps, l’eau le recouvre ; la hiérarchie lumineuse de la charte tient.',
      'LA LUEUR DES GOUTTES PERDUES : une goutte détachée du corps portait jusqu’ici « eau libre plus sombre » dès la première image — du décor. Elle garde maintenant un grain de SA lumière, posé sur elle tant que le champ la porte : vive à l’instant où elle part, elle S’ÉTEINT avec son délai de réabsorption (au carré : vite d’abord, puis lentement), et il reste une braise (0,3) tant qu’elle est dans le halo de rappel — elle reviendra. Sortie du halo, plus rien : elle est devenue de l’eau. Ce sont les données du solveur (cooldown, marque « du corps » ouverte en lecture), pas une simulation de plus. Au plan large, les motes s’effacent (sous ~1,3 px de rayon) ; la lueur d’une goutte reste un voyant.',
      'VÉRIFIÉ : 9 tests neufs sur la vie (l’accrochage aux gouttes du corps seulement, le suivi du fluide, la relève quand l’hôte est éjecté, le resserrement sous la peur, l’extinction à la dispersion, la lueur fraîche / expirée / braise, le tampon et sa capacité) — 1082 verts dans 101 fichiers, type-check à 0, build propre. Les cinq programmes GLSL (dont la passe de vie et la composition avec le halo) COMPILENT ET SE LIENT dans un vrai WebGL2 (Chromium + SwiftShader, page d’essai) ; le jeu construit tourne en Chromium : WebGL2 actif, 900 gouttes, les 44 motes accrochées au corps à 0,99 d’opacité, zéro erreur de console. NON VU D’ICI : l’image elle-même (l’onglet tombe à la capture en rendu logiciel) — le dosage des trois effets se juge en prévisualisation (git push -f origin claude/volume-personnification-tkfh7a:previsu-go).',
    ],
  },
  {
    date: '11/09/2026 11:51',
    title: 'L’AVIS SUR UNE MUSIQUE : +1, −1 ou neutre au bout de la ligne ÉCOUTE, un par personne, partagé',
    notes: [
      'LA DEMANDE : sur le lecteur de l’accueil, « pouvoir mettre un +1 ou −1 ou neutre » — pour qu’au fil des musiques on marque celles qui sont bien et celles qu’on ne veut pas, partagé. Un premier jet posait UN verdict par piste, le dernier l’emportant ; la réponse a préféré un avis PAR PERSONNE avec le total en face — c’est ce qui est livré.',
      'TROIS TOUCHES au bout de la ligne ÉCOUTE (−1, ○, +1), éteintes tant que rien ne joue. La touche enfoncée est MON avis, à la couleur de son verdict (vert : retenue, rouge : écartée) ; le titre dit le total et qui pense quoi : « 4/14 · Cuve tiède (coque chaude) · +1 · JULIEN +1, MARIE +1, PAUL −1 » — la ligne se coupe, l’infobulle la garde entière. À l’arrêt, le bilan : « 3 retenues, 2 écartées, 1 partagée » (retenue si la somme est positive, écartée si négative, partagée si les avis se neutralisent).',
      'UN AVIS PAR PERSONNE ET PAR PISTE : la personne est le nom de la borne (celui des records) ; sans nom, « anonyme », et tous les anonymes ne font qu’un — le prix d’un nom qu’on n’a pas donné. Le NEUTRE n’est pas une valeur rangée : c’est l’absence de ligne — se remettre au neutre retire le sien et rien d’autre.',
      'PARTAGÉ (game/ecouteAvis.ts, domaine « ecoute » du magasin /api/reglages — pas de fonction Vercel de plus, le plan Hobby en compte douze) : chargé à l’ouverture de l’accueil en mode concepteur, jamais par un joueur. La publication est différée de 800 ms et regroupée : trois touches coup sur coup ne coûtent qu’une écriture au magasin (2 put + 1 list). Avant d’écrire, on relit le publié pour poser nos avis PAR-DESSUS, entrée par entrée : deux concepteurs qui trient en même temps ne s’effacent pas — à la fenêtre près du cache de lecture, 15 s, accepté. Hors ligne ou refusé, les avis restent en main, le titre dit « avis non publié », la prochaine touche retente.',
      'LE SERVEUR ne connaît pas la forme : la relecture ramène le document dans ses bornes — seules les pistes que l’écoute connaît comptent (un avis sur un fichier disparu ne survit pas), seuls +1 et −1 sont des avis, un auteur tient en quarante signes et n’est jamais vide.',
      'VÉRIFIÉ : six tests sur l’avis (chacun pose le sien et le total les somme, le neutre retire le sien seulement, l’anonyme, les bornes de relecture, l’aller-retour JSON, le bilan et les lignes du titre). 1073 tests verts dans 100 fichiers, type-check à 0, build propre. Comme pour le lecteur, le jeu complet ne se monte pas dans le Chromium sans tête de l’environnement : les touches n’ont pas été cliquées dans un navigateur ici — à voir sur l’aperçu (previsu-go).',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : « est-ce qu’il ne serait pas possible d’avoir quelque chose de mieux pour le fantôme ? là on a uniquement sa position, c’est pas dingue ». Trois choses, d’un coup, par ordre de rapport entre ce qu’elles apportent et ce qu’elles coûtent.',
      'LA FORME (game/fantome.ts, version 2 de la trace) : à chaque échantillon, l’étendue du corps dans SEIZE SECTEURS autour de son centre — un octet par secteur, au quart d’unité. La silhouette devient une goutte qui s’étire, se creuse quand elle éjecte, se tasse en bloc, se gonfle en nuage : une courbe lissée par les milieux des côtés, avec le dégradé et le liseré du fluide, pointillée en eau, pleine en glace, voilée en vapeur. Le calcul du profil (profilDe, pur, testé) parcourt les particules dix fois par seconde simulée ; un secteur vide se comble par ses voisins, la silhouette ne se déchire pas pour un trou d’échantillonnage. Les traces de version 1 (huit octets) se relisent toujours — en cercle.',
      'LE GESTE : la direction de POUSSÉE quand le joueur éjecte (un octet, l’angle sur 254 crans, 0 = il n’éjecte pas), et les DASHS de vapeur en événements datés ([t, type, angle] à plat, à côté de la trace). À l’écran, le fantôme crache son jet de gouttelettes vers le point qu’il visait (le corps recule, comme le vrai), un trait blanc-violet file au dash, et chaque bascule d’état fait un éclair — anneau franc pour la glace, halo pour la vapeur, ondulation pour l’eau. Les bascules se lisent dans la trace elle-même : pas d’événement à stocker.',
      'LA LIGNE DE COURSE : toute la trace en filigrane dès l’entrée dans la salle, pointillée, colorée par état — on lit la stratégie du record avant de jouer. Un point sur k pour rester sous six cents segments par fantôme.',
      'L’ÉCART EN DIRECT, dans la sous-ligne du HUD : « VOLUME REX +1,2 s · −0,28 L ». Le temps, façon jeu de course : à l’instant t je suis à telle distance du sas — combien de temps le fantôme a-t-il mis pour être aussi près ? La distance retenue est la MEILLEURE atteinte jusqu’à chaque échantillon (un aller-retour ne compte pas deux fois), la recherche est dichotomique. « devant » quand le fantôme n’a jamais été aussi près. Le volume : le mien contre le sien au même instant. Fantôme au sas : « au sas en 1:12 · 3,40 L ».',
      'LE POIDS : vingt-cinq octets par échantillon au lieu de huit — une course de trois minutes pèse ~60 Ko encodée (mesuré par le test). Le budget du rangement passe à 1,5 million de caractères : une vingtaine de courses, les plus anciennes partent d’abord. Ce budget est une SOUPAPE : le navigateur plafonne le stockage d’un site vers 5 Mo, sauvegarde comprise ; sans lui, une centaine de records ferait déborder l’emplacement et l’écriture échouerait en silence. Une coquille qui écrit dans un fichier pourra relever la barre.',
      'VÉRIFIÉ : 958 tests verts dans 86 fichiers (8 pour le fantôme : l’aller-retour à vingt-cinq octets et ses bornes, la version 1 relue en cercle, le profil et ses secteurs vides, l’enregistreur et ses événements, le lecteur qui interpole la forme et garde le geste le plus proche, l’écart de temps avec l’aller-retour qui ne compte pas deux fois, le rangement avec événements et traces d’avant, le budget et le poids réel de trois minutes), type-check à 0, build propre. Au navigateur (Chromium, jeu construit, salle 21-01) : un fantôme allongé posé et relu au R, dessiné à l’étendue de son profil (68 px de rayon à ce zoom, soit 140 unités : la forme commande, pas le rayon moyen), 4 000 pixels peints dans son cercle contre 0 masqué, le HUD qui dit « VOLUME REX 0,0 s · +1,50 L » — zéro erreur console. Non vu d’ici : le jet, le dash et les éclairs EN MOUVEMENT (le temps simulé n’avance presque pas dans l’environnement d’essai), à voir en prévisualisation.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : « on pourrait déjà faire un système de fantômes partageable ou non », puis « tu peux enchaîner » après le coffre. Voici la version LOCALE : le fantôme de vos propres records, dans votre emplacement — la version partagée viendra avec la cloison réseau.',
      'PAS UN REJEU DES ENTRÉES. La simulation n’a aucun aléa, mais le solveur appelle vingt-neuf fois des fonctions (puissance, exponentielle, sinus) dont le dernier bit varie d’un navigateur à l’autre, et le test de parité JS ↔ WASM tolère déjà un écart qui grandit avec le temps. Rejouer les entrées d’un joueur sur une autre machine donnerait un autre film au bout de quelques secondes. On enregistre donc LA TRAJECTOIRE DU CORPS, comme les jeux de course : centre, rayon quadratique moyen, volume et état, dix fois par seconde de temps SIMULÉ — la pause fige le fantôme, le time warp le presse, les records ne bougent pas.',
      'LE MODULE (game/fantome.ts, pur, testé) : l’ENREGISTREUR échantillonne à cadence fixe, calé sur la grille (un pas en retard ne décale pas la trace) ; l’encodage tient en HUIT OCTETS par échantillon (x et y en entiers 16 bits, le rayon au demi, les centilitres, l’état) en base64 — une course de trois minutes pèse ~19 Ko ; le LECTEUR interpole entre deux échantillons et disparaît après le dernier : le fantôme a bu le sas, il n’est plus là ; le RANGEMENT garde un fantôme par salle et par record (VOLUME, CHRONO), dans l’emplacement de sauvegarde — donc dans le fichier que le Steam Cloud synchronisera — sous un budget de 1,2 million de caractères : au-delà, le plus ANCIEN part, jamais celui qu’on vient de poser.',
      'DANS LE JEU : à chaque entrée de salle (restart), l’enregistreur s’arme et les fantômes de la salle se relisent ; au pas de simulation, un échantillon par cran de cadence — le parcours des particules n’a lieu que dix fois par seconde simulée ; au sas, si le record de VOLUME ou de CHRONO tombe, la trace de la course prend la place de l’ancienne (la même course qui prend les deux ne fait qu’une silhouette). Ni le hub ni l’essai d’éditeur n’enregistrent : ils ne consignent pas de record. RÉINITIALISER L’OPÉRATEUR emporte les fantômes avec les records.',
      'LE DESSIN, sur le canvas d’effets (celui des portes et de la flèche) : une silhouette en dégradé à 1,4 fois le rayon quadratique moyen (le rms d’un disque plein vaut R/√2 : c’est le bord du corps qu’on retrouve), teintée selon l’état — bleu d’eau, blanc-bleu de glace, blanc-violet de vapeur —, pointillée, avec son étiquette « VOLUME · REX » ou « CHRONO · REX ». Le fantôme du volume est gros et lent, celui du chrono petit et vif : on voit l’arbitrage du jeu incarné par une course d’avant. PARAMÈTRES › LES FANTÔMES : VISIBLES ou MASQUÉS, réglage de l’appareil.',
      'VÉRIFIÉ : 950 tests verts dans 85 fichiers (5 neufs : l’encodage aller-retour et ses bornes, l’enregistreur à cadence fixe et sa grille, le lecteur qui interpole et disparaît, le rangement relu depuis le stockage et ses textes cassés, le budget qui évince les plus anciens), type-check à 0, build propre. Au navigateur (Chromium, jeu construit, salle 21-01) : l’enregistreur échantillonne pendant la partie, un fantôme posé se range dans le document de l’emplacement 1, R le relit (une silhouette VOLUME), et le canvas d’effets porte 8 787 pixels peints dans son cercle contre 0 une fois masqué — zéro erreur console. Non vu d’ici : une vraie course jusqu’au sas (l’environnement d’essai simule trop lentement), à jouer en prévisualisation.',
    ],
  },
  {
    date: '06/09/2026 00:04',
    title: 'LE COFFRE : trois emplacements de sauvegarde — un document par emplacement, prêt pour le Steam Cloud',
    notes: [
      'LA DEMANDE : « je souhaite également faire le vrai système de sauvegarde avec 3 slots possible ; il faudra que cela soit compatible avec les sauvegardes Steam ». Avant le fantôme : le fantôme est une donnée de progression, autant poser le socle d’abord.',
      'LE CONSTAT : « le profil », c’était le navigateur — 58 clés de stockage dans 21 fichiers, trois conventions de nom, et trois natures mêlées : la progression du joueur, les préférences de l’appareil, les brouillons du concepteur. Un emplacement n’existait pas en tant qu’objet : impossible de recommencer une partie sans perdre l’autre, impossible de porter sa progression ailleurs.',
      'LE COFFRE (game/coffre.ts, pur, sans dépendance) : trois emplacements, UN document JSON par emplacement — format, date d’écriture, appareil qui a écrit, et les clés des modules comme sections. Les modules de progression n’ont pas changé de contrat : ils lisent et écrivent leurs clés d’avant à travers la FAÇADE de l’emplacement actif (coffre.stockage) — registres, codex et ses objectifs, trophées, palmarès et butin de la voie, éveil, run en cours, prise en main, cinématiques vues, signature, condensat hérité, réinitialisation de l’opérateur. Les préférences (résolution, cadence, commandes, son) et les brouillons du concepteur restent au stockage local : ils ne suivent pas le joueur.',
      'STEAM CLOUD : le document est fait pour être un fichier (Auto-Cloud : un dossier, trois fichiers). Il s’écrit EN ENTIER à chaque changement, jamais par morceaux ; il porte un numéro de format et une chaîne de migrations ; un document d’une version PLUS RÉCENTE du jeu se lit en lecture seule et ne s’écrase jamais ; la coquille native se branchera par window.sujet21Hote.coffre (stockage synchrone, écriture atomique : fichier temporaire puis renommage), sans une ligne d’API Steam dans le jeu.',
      'L’HÉRITAGE : au premier lancement, les clés de progression d’avant deviennent l’emplacement 1, une seule fois (marqueur). Les clés d’origine restent en place une version, en filet ; une version ultérieure les balaiera. Une clé neuve n’a pas à figurer dans la liste : elle naît dans le coffre.',
      'L’ÉCRAN SAUVEGARDES (accueil, bouton ▤) : trois cartes — opérateur, essais, mémoire, salles au record, fioles, trophées, run en cours, date et poste du dernier enregistrement ; l’active marquée EN COURS. JOUER écrit le pointeur et RECHARGE (tout l’état de main.ts se construit à l’ouverture depuis l’emplacement actif, on ne le rebâtit pas à chaud) ; EFFACER se confirme en deux clics, comme la réinitialisation de l’opérateur ; EXPORTER livre le document tel qu’écrit ; IMPORTER le repose, et recharge si c’est l’actif. B referme le voile à la manette, comme les autres.',
      'VÉRIFIÉ : 945 tests verts dans 84 fichiers (11 neufs : façade et document écrit d’un bloc, migration de l’héritage qui ne se rejoue pas et n’emporte ni préférence ni brouillon, toutes les clés de progression, changement d’emplacement étanche, lecture seule d’un document plus récent, textes invalides, export → import, effacement, dos qui refuse d’écrire), type-check à 0, build propre. Au navigateur (Chromium, jeu construit) : un joueur d’avant migré dans l’emplacement 1 sans re-signer, l’écran et ses trois cartes, JOUER sur le 2 → partie vierge et signature demandée, la signature écrite dans le 2 seul et le 1 intact, EFFACER en deux clics, l’export en JSON au format 1, l’import dans le 3 — zéro erreur console. Non fait, et c’est voulu : le balayage des clés d’origine (une version plus tard) et la coquille native elle-même.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : « il faut que je puisse gérer le nombre de fins nécessaire, en définissant l’ordre souhaité » — et, plus large, regrouper les outils du concepteur autour du système de run. C’est l’étape 1 du plan proposé : le journal en données, parce que le récit et les fins n’ont PAS de détecteur dans le code (ils se servent dans un ordre, à l’expédition bouclée) — rien n’oblige à les figer, contrairement aux fiches d’expérience.',
      'LE MODÈLE (game/journal.ts, testé) : un journal = le récit (fragments ordonnés), les fins (ordonnées), la RÉVÉLATION après N fragments, le DÉNOUEMENT après N fins. Le journal LIVRÉ (dix fragments, une fin, 10 et 1) vit dans le code comme filet ; le journal COURANT est celui publié au magasin partagé (/api/journal, Vercel Blob, dernier écrivain gagnant) dès qu’il arrive — hors-ligne, le livré joue. Les fiches du codex ne sont plus une liste fixe : fichesCodex() = les expériences (CODEX_EXPERIENCES, code) + le journal courant. Deux règles gravées dans les tests : un id ne se réattribue jamais (les registres des joueurs s’en souviennent), fin-jouee est réservé ; les seuils se comptent en entrées SERVIES, pas en position — retirer une entrée déjà lue ne retire rien à personne.',
      'LES REGISTRES : le récit se notait par son nom court (« livraison »), les fins par l’id de leur fiche ; désormais tout se note par l’id de la fiche (« recit-livraison ») — une seule clé, celle du codex. Les registres d’avant se relisent à l’identique par une migration au chargement.',
      'LE JEU : l’expédition bouclée sert prochainFragment puis prochaineFin du journal courant ; la révélation (le sceau tombe) lit revelationAtteinte ; l’alcôve du secteur 4 ne joue la cinématique que si denouementAtteint — avant, un toast dit combien de fins manquent au lieu de rester muette. Le scénario gagne deux conditions, « revelation » et « denouement », qui lisent ces seuils : la règle livrée passe de « jalons ≥ 10 » à « revelation », plus aucun N à tenir en double (un scénario déjà publié garde sa règle telle quelle).',
      'L’ATELIER (editor/atelierJournal.ts, accueil RÉCIT & FINS ou bouton dans le rayon du journal au codex) : deux colonnes, LE RÉCIT et LES FINS ; chaque entrée se glisse (poignée ☰) ou se monte/descend, s’écrit (icône, titre, texte), se retire (confirmation : les joueurs qui l’ont lue la perdent du codex) ; + AJOUTER dérive un id du titre, unique, figé et affiché. Les deux seuils se règlent en tête, bornés au nombre d’entrées, rabattus si on retire. Un BROUILLON par poste (localStorage, survit au rechargement) ; PUBLIER (grisé tant que le brouillon a une erreur ou vaut le publié) ; ESSAYER SUR CE POSTE fait jouer le brouillon ici sans publier ; RECHARGER, REVENIR AU PUBLIÉ, BROUILLON = LIVRÉ, RETIRER LE PUBLIÉ. Les verdicts nomment ce qui cloche (id en double, texte vide, seuil au-delà du nombre) et distinguent l’attention de l’erreur. Une frappe ne repeint que l’en-tête : le curseur reste dans la phrase.',
      'LES TEXTES : le catalogue ne liste plus que les fiches d’expérience — le journal s’écrit dans son atelier, comme les tableaux de la bibliothèque, dans la langue de son auteur (le garde-fou du catalogue passe à 450 entrées : 499 mesurées, 22 de moins qu’avant). decouvertes.ts disparaît, remplacé par journal.ts.',
      'CE QUI SUIT (étapes 2 et 3 du plan) : la coque LA RÉGIE qui regroupe carte, descente, planche, récit & fins, scénario, codex, marchand, récompenses ; puis le tir à blanc du système entier. Et les fins alternatives (evil, semi-evil…) s’écrivent maintenant dans l’atelier, sans toucher au code.',
      'LA REVUE DE LA PR a relevé trois choses, corrigées : une retouche de l’atelier des TEXTES faite AVANT (quand le récit était au catalogue) passait encore par-dessus le titre écrit dans l’atelier du journal — invisible et impossible à retirer ; codexLu ignore désormais toute retouche pour le récit et les fins (testé). Une carte toujours « draggable » vole la sélection de texte dans ses champs sous Firefox : elle ne se glisse plus que par sa poignée. Un seuil laissé vide reprend au blur ce que le brouillon a retenu.',
      'VÉRIFIÉ : 889 tests verts dans 78 fichiers (journal livré, lecture d’un journal reçu, opérations de l’atelier, seuils, courant ; codex dynamique ; scénario), type-check à 0, build propre, l’API type-checkée à part ; au navigateur (Chromium, page d’essai, magasin simulé) : dix fragments et une fin, ajout d’une fin (id fin-la-fin-evil, verdict « texte vide » puis levé), monter, seuil à 2 puis borné, glisser-déposer par la poignée (événements de drag), publier puis relecture (le courant compte deux fins), revenir au publié, brouillon gardé au rechargement, retirer le publié (le courant revient à une fin), retrait d’un fragment qui rabat le seuil — zéro erreur console. Le magasin réel et le déclenchement en jeu restent à voir en prévisualisation.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'L’ÉTAPE 2 DE LA CARTE (docs/carte-station.md), empilée sur l’éditeur : la descente n’est plus une FILE de douze rangs avec un module toutes les n salles, c’est le PLAN DE LA STATION qui la conduit. Un module est un biome ; au bout de ses salles la carte se déploie dans la cérémonie ; la coursive suivante se choisit d’un clic ; la longueur d’une run DÉCOULE du trajet.',
      'L’ÉTAT VIT DANS descenteCarte.ts, pur et testé (neuf tests) : le module où l’on joue, les salles déjà franchies dedans, les modules traversés. departCarte, moduleFini, choixModules (les modules au bout d’une coursive, avec l’orbe qui manque), entreModule (refusé sans coursive ouverte), franchitSalle, objectifAtteint (le module objectif, épuisé — c’est la fin de l’expédition), longueurRun. L’état s’écrit dans la sauvegarde de run et se relit à la reprise ; une sauvegarde d’avant la carte, ou qui cite un module disparu, repart du départ : jamais une run bloquée.',
      'LA LONGUEUR N’EST PLUS UN RÉGLAGE. longueurRun = salles franchies + salles restantes du module + plus court chemin en niveaux jusqu’à l’objectif (plusCourtVers, un Dijkstra sur une dizaine de sommets). Elle s’affine à chaque coursive choisie et n’est jamais plus petite que le rang. Le plan de voie garde tout le reste — rampe, moments, postures, pioche — et reçoit cette longueur vraie (planEffectif) : momentAuRang, diffAuRang, reglageAuRang, ampleurAuRang, le HUD, le dossier, l’Économat de mi-descente, tout suit. Sur la carte livrée : 9 salles par la voie principale.',
      'LA CARTE DANS LA CÉRÉMONIE. Au sas de lancement, le plan s’ouvre (le panneau s’élargit) : le premier module se choisit sur la carte, plus dans la bibliothèque. Un module fermé dit l’orbe qui manque et secoue la scène ; un module hors de portée dit qu’aucune coursive n’y mène ; le survol (ou le focus, à la manette) lit sa fiche sur une ligne. Le module élu S’AGRANDIT — la scène zoome sur lui, origine de la transformation posée sur ses coordonnées — puis ses salles arrivent en vignettes, le choix habituel de la voie titré du nom du module et de la salle dans le module (« AUCUNE TRANSFO — SALLE 2 / 3 · DESCENTE 5 / 9 »). Un NŒUD (sans salle) rouvre la carte aussitôt. Un écran, deux temps, comme le concepteur l’a voulu.',
      'LA PIOCHE SUIT LE BIOME. Un tableau gagne un champ `biome` (lu et écrit avec le tableau) : s’il en porte un, il ne se propose que dans le module de ce biome ; sans biome, il est universel — la bibliothèque reste jouable tant qu’elle n’est pas réétiquetée (étape 4). Une salle générée prend le biome du module.',
      'LES ORBES, EN ATTENDANT LES ORBES : un orbe est tenu pour acquis quand la transformation qu’il nomme est tissée au cycle des mémoires (et un orbe d’état quand une transformation qui y mène l’est) — la règle qui valait déjà pour la pioche, appliquée aux coursives. L’étape 3 branchera l’inventaire réel.',
      'L’ÉCRAN LA STATION LIT LA CARTE : position, modules traversés, coursives ouvertes ou sous cadenas, fiche du module visé (zone, état, description, nature, température, salles), légende tirée des types de coursive. Le plan linéaire (station.ts, planStation.ts) n’est plus importé nulle part — les fichiers restent au dépôt, à retirer par un humain (leur suppression a été refusée à l’outil). Les outils (crochet __expedition, files d’essai) lancent une run sans carte : elle entre d’office par la première coursive ouverte et joue la salle 1 comme avant.',
      'AU PASSAGE, LA REPRISE MENTAIT : la sauvegarde ÉCRIVAIT le rang, la bourse de condensat, la mémoire gagnée et la visite de l’Économat, mais runSauvee ne les RELISAIT pas — la reprise repartait au rang de l’index, bourse vide, Économat oublié. Les quatre champs reviennent, avec la carte.',
      'VÉRIFIÉ : 850 tests verts dans 74 fichiers, type-check à 0, build propre. PAS VU AU NAVIGATEUR : le jeu complet ne s’affiche pas dans le Chromium sans tête de l’environnement d’analyse (rendu logiciel, la page ne monte jamais) — seule la couche pure est éprouvée par les tests, et l’écran de la cérémonie, LA STATION et le HUD n’ont été vérifiés que par le type-check. À regarder en vrai avant la PR vers prod : le sas de lancement, un module traversé jusqu’au bout, un nœud, un cadenas, la reprise d’une run sauvée.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : la carte de la station dessinée par le concepteur (handoff « Carte de la station », onze modules reliés par douze coursives dans quatre zones, avec des conditions d’accès selon l’état du sujet — eau, glace, vapeur) doit devenir la carte du jeu. Et AVANT la carte, un éditeur : pour que la construire ne soit pas un travail de développeur, et pour que le concepteur la fasse évoluer ensuite lui-même.',
      'CE QUI MANQUAIT. Le §9 du document fonctionnel réclame la ramification à la Dead Cells depuis le premier jour, et le README l’avouait : « il n’y a pas d’écran de carte ». Le plan de LA STATION (station.ts) est une FILE — six modules sur une poutre, un module toutes les n salles. Une carte à routes ramifiées ne se recopie pas en code sans la figer : la prochaine évolution redemanderait un développeur.',
      'LA SOURCE DE VÉRITÉ EST UN FICHIER JSON. src/game/carteStation.json est la carte du concepteur, copiée telle quelle (map.json du handoff). Le jeu la lit à travers carteStation.ts — types, lecture qui NOMME ce qui manque (« modules[2] (T2) : x (nombre) requis »), sérialisation dans le MÊME ordre de clés (un export relu donne un diff vide, un diff ne montre que ce qui a changé), et vérification de fond : identifiant en double, coursive vers un module inconnu, module que nulle route n’atteint, objectif hors de portée, deux modules qui se chevauchent. Rien de la carte n’est écrit en dur — ni couleurs, ni positions, ni règles.',
      'LE DESSIN EST PUR, ET EN SVG — même parti que le plan de la station, pour les mêmes raisons : ça se survole, se clique, se glisse, porte du texte qui doit rester net, et ses animations tiennent en CSS. dessinCarteSVG rend une chaîne depuis les données et un état de lecture ; il reprend la maquette avec une fidélité haute (octogone 22/78/28/72, rond pour la jonction, dôme pour le terminal, coursives en trois traits — paroi, sol, anneaux en tirets « 4 44 » — puis la ligne de route colorée par type, badges ❄ GLACE et 💨 GAZ, arc de coque et télescope amarré). TOUT tient dans un seul viewBox : la maquette mêlait HTML et SVG et mettait la scène à l’échelle à la main ; ici le navigateur fait l’échelle, et l’éditeur convertit un pointeur en coordonnées de scène par la matrice du SVG, sans rien calculer.',
      'LES NERVURES DE L’ARC SONT CALCULÉES SUR SES COURBES, pas posées à la main comme dans la maquette : l’arc peut changer de forme dans le JSON sans qu’on les refasse. Et la règle des coursives du hub — « sortie à y = clamp(cible.y, HUB.y − 110, HUB.y + 110) » — n’a plus de nombre magique : 110, c’est h/2 − 36 pour un fût de 292. Elle s’applique à tout module plus haut que large, à chaque bout ; un second hub se comportera comme le premier. Trois tests la gravent (sortie à 270, 385 et 490 vers T1, T2, T3).',
      'L’ÉDITEUR, MÊME ARCHITECTURE QUE CELUI DES TABLEAUX : une barre qui s’enroule, deux colonnes bornées au tiers de l’écran, la scène au milieu. Glisser un module (centre aimanté à la grille, 8 px), tirer un coin pour le redimensionner (le coin opposé ne bouge pas, 32 px minimum, et l’aimant ne DÉRIVE pas image après image parce qu’on recalcule depuis l’origine du geste — testé sur trente pas d’un demi-pixel), tracer une coursive d’un module à un autre (Lier, L, ou Maj + glisser ; la ligne brouillon suit le doigt, et c’est la GÉOMÉTRIE qui dit sous quel module on lâche, car sous le doigt se trouve la ligne, pas le module), renommer un identifiant (les coursives, le décor ancré, le départ et l’objectif suivent), supprimer (un module emporte ses coursives), inverser le sens d’une coursive, changer son type, Ctrl+Z / Ctrl+Y — UNE étape par geste, pas par image.',
      'LES GESTES SONT PURS ET TESTÉS (carteOperations.ts, seize tests) ; le DOM ne fait que traduire. Le formulaire est GÉNÉRIQUE : un champ, un chemin (« modules.3.temp », « typesLiens.glace.condition »), et poseChamp garde le type en place — un nombre reste un nombre, une saisie qui n’en est pas un est refusée et le champ rougit, une condition vide redevient un passage libre. Zones et types de coursive (couleur, trait, coque, tirets, condition, badge) s’éditent sans sélection ; on en ajoute.',
      'L’APERÇU JEU REJOUE LE COMPORTEMENT VOULU IN-GAME sans quitter l’éditeur : le sujet part du module de départ, seuls les bouts des coursives partant de sa position sont accessibles, une condition non remplie montre un cadenas et un bouton ACCÈS REFUSÉ, ENTRER avance, la coursive parcourue s’efface à 35 %, le module courant pulse. On bascule l’état du sujet (eau · glace · vapeur) pour vérifier que chaque route s’ouvre comme prévu — une carte se traverse avant d’être livrée.',
      'LA VÉRIFICATION EST VISIBLE EN PERMANENCE, dans la colonne de gauche : chaque verdict se clique et sélectionne le fautif. Exporter télécharge carteStation.json, à déposer par-dessus src/game/ — le test de carteStation.spec.ts relit le fichier du dépôt et REFUSE une carte illisible ou avec une erreur de fond : le jeu ne démarrera pas sur une carte muette. Le document est retenu sur le poste d’une séance à l’autre ; « Carte livrée » ramène au fichier du dépôt, annulable.',
      'CE QUI N’EST PAS FAIT, ET C’EST VOULU : la carte n’est PAS encore branchée dans le jeu. L’écran LA STATION lit toujours le plan linéaire, et passer d’une coursive choisie à la salle suivante demande une décision de conception (le générateur pioche par moment · mécanique ; un module pourrait porter ce code) — docs/carte-station.md la pose. dessinCarteSVG en mode jeu est prêt pour cet écran.',
      'VÉRIFIÉ : 836 tests verts dans 73 fichiers (48 neufs — lecture, sérialisation à l’identique, règle du hub, conditions, échelle de température, vérification, dessin, gestes, historique), type-check à 0 erreur, build propre. ET AU NAVIGATEUR (Chromium piloté, page d’essai n’important que l’éditeur — le jeu complet ne tourne pas en rendu logiciel) : glisser, Ctrl+Z, redimensionner par une poignée, Maj + glisser d’un module à l’autre, changement de type dans la fiche, renommage suivi par les coursives, verdicts, aperçu jeu (T1 refusée en état eau, ouverte en état glace, ENTRER avance), export du fichier, réouverture sur le document retenu, retour à la carte livrée, import refusé avec la liste des défauts — zéro erreur console. C’est là qu’un défaut de dessin s’est vu : un clip-path posé sur un <svg> imbriqué est lu par Chrome dans le repère du viewBox intérieur, et le fût du HUB visité disparaissait sous une plaque bleue pleine ; la découpe est désormais sur un groupe sans transformation, l’étirement sur un groupe dedans.',
    ],
  },
  {
    ...
    notes: [
      ...
      'LA RÈGLE EST EN DEUX TEMPS : un seuil d’amorçage FRANC, et un renflouement PROGRESSIF. Sous 30 % du volume de départ, le renflouement s’engage ; il rend ensuite une DOSE par repos — au plus 8 % du plein — jusqu’à ce que le corps soit refait, puis se relâche. Trente pour cent parce qu’au-dessus le corps n’aurait plus le droit de maigrir et le hub perdrait sa nervosité ; au-dessous, on retomberait dans la zone d’alerte que ce mécanisme existe pour éviter. Huit pour cent parce qu’au-dessus le saut de propulsion se sent, et qu’au-dessous le débit ne suivrait pas une poussée soutenue, qui dépense environ 3,5 % du corps par seconde.',
      'L’HYSTÉRÉSIS N’EST PAS UN RAFFINEMENT. Avec un seuil unique, la première dose repasserait au-dessus de 30 % et le versement s’arrêterait là : le corps vivrait collé à son seuil, à un souffle de l’alerte. On s’amorce donc à 30 %, on relâche au PLEIN, et un état empêché (glace, vapeur, pause) ne relâche PAS le renflouement en cours — sinon un passage en glace l’annulerait au milieu.',
      'MESURÉ, vingt-cinq secondes au hub, poussée continue puis repos : le corps descend 100 → 83 → 63 → 43 %, le renflouement s’engage à 33 %, et la remontée se fait par paliers jusqu’à 100 %. Vingt versements, SAUT MAXIMAL 8 % DU PLEIN — contre 83 points d’un coup avant. Le volume de départ, lui, ne bouge plus : base 900 du début à la fin, jauge d’un corps plein à 100 %.',
      ...
      'LE CORPS GARDE LE DROIT DE MAIGRIR, et c’est voulu : jusqu’à 30 %, rien ne se verse, donc la nervosité d’un corps léger reste jouable au hub. Ce qui disparaît, c’est le retour brutal — pas la légèreté.',
      'VÉRIFIÉ : 788 tests verts (8 neufs), type-check à 0 erreur, build propre. Le test du volume de départ a été éprouvé DANS LES DEUX SENS — défaut remis, il tombe sur « expected 321 to be 300 » ; correctif remis, il passe.',
    ],
  },
  {
    ...
    notes: [
      'LA DEMANDE : un écran de joueur, la carte globale du jeu — une station modulaire dans laquelle on progresse au fil de la run, d’une case tous les x niveaux, un module valant un ensemble de niveaux : un biome.',
      'CE QUI MANQUAIT. La bible le réclame depuis le premier jour : « chaque tableau EST un module ou une parcelle de module », « on doit reconnaître la station comme on reconnaît l’ISS sur une infographie ». Or la descente n’était qu’une SUITE — un rail de crans sur le flanc droit, « salle 5 / 12 ». Le joueur avançait dans un compte, pas dans un lieu.',
      'SIX MODULES, et le chiffre n’est pas décoratif : six se coupe par DEUX comme par TROIS. À douze salles on avance d’un module toutes les deux, à dix-huit toutes les trois — et dans les deux cas les tiers de la descente (début · milieu · fin) tombent EXACTEMENT sur une frontière de module, deux modules par moment. Aucun autre compte ne donne les trois à la fois, et un test le grave sur quatre longueurs : jamais un module à cheval sur deux moments, sans quoi le biome changerait au milieu d’un rang.',
      'UN MODULE EST UN BIOME. L’AMARRAGE (parois nues, hydrophobe) enseigne le geste ; LES SERRES retiennent (hydrophile, membranes) ; LES CRYOBAIES figent ; LA FERME est la partie non pressurisée, grilles et évents, où seul le souffle passe ; LE BLOC THERMIQUE est l’endroit le plus chaud de la station ; LE SECTEUR 4 est scellé, rails de champ et convoyeur. La montée des matières suit celle des mécaniques du plan — eau, puis glace, puis vapeur, puis tout — et un test refuse qu’un module en redemande moins que le précédent.',
      'LE PLAN, DESSINÉ. Une poutre verticale, quatre panneaux solaires, des radiateurs, et la file des modules amarrés bout à bout depuis MÉDUSE : la silhouette d’une station, pas une rangée de cases. Le module franchi s’allume, le module courant respire (halo lent, marqueur au-dessus du fût), la route parcourue file en pointillés depuis le hub, un fanion marque le plus loin jamais atteint sur ce poste, et le secteur 4 porte ses barres de scellé tant que tout n’est pas raconté. Le ciel est un semis d’étoiles à graine FIXE : un fond qui changerait à chaque ouverture se remarquerait, et pour rien.',
      'EN SVG, ET C’EST UN CHOIX. La mini-carte des tableaux (carte.ts) est un canvas ; ce plan-ci se survole, se clique et se tabule, il porte du texte qui doit rester net à toutes les tailles, et son animation tient en trois lignes de CSS. Un canvas aurait redemandé un pipeline de dessin, des tests de collision au pointeur et une gestion du zoom — tout ce que le navigateur fait déjà. Le module de dessin est PUR : il rend une chaîne, ne touche pas au DOM et ne lit pas l’état du jeu, donc il se teste sans navigateur (dix tests).',
      'LE PLAN NE SE RECONSTRUIT PAS AU SURVOL : le SVG est bâti une fois à l’ouverture, viser un module ne fait que basculer une classe et repeindre la fiche du bas. Rebâtir le dessin à chaque mouvement du pointeur relancerait toutes les animations et volerait le focus au clavier. Souris, clavier et manette visent par le même chemin — le plan ne se lit pas qu’à la souris.',
      'IL MET LA PARTIE EN PAUSE, contrairement au dossier de descente (Tab) qui ne fige rien. La raison est simple : le dossier est un panneau de côté, le plan est un VOILE qui couvre tout l’écran — et le corps dérive derrière. La pause n’est rendue à la fermeture que si c’est le plan qui l’a posée : on ne réveille pas une partie que le joueur avait figée exprès.',
      'ET LE MODULE ATTERRIT DANS LE JEU, pas seulement dans son écran : la carte d’entrée de salle le nomme désormais (« SALLE 5 / 12 · LES CRYOBAIES »). Sans cela, le biome n’aurait été qu’un dessin.',
      'TROIS DÉFAUTS D’AFFICHAGE TROUVÉS AU BANC, avant livraison, et corrigés : les noms de modules se recouvraient d’un fût à l’autre (« LE NŒUD D’AMARRAGE » mangeait « LES SERRES ») — ils se coupent maintenant en deux lignes équilibrées ; les radiateurs du bas et les panneaux solaires tombaient dans la bande des étiquettes et se lisaient comme des modules sans nom — ils l’encadrent désormais sans jamais l’écrire ; et le marqueur « vous êtes ici », posé au centre du fût, se confondait avec le hublot — il est monté au-dessus, avec sa flèche.',
      'CE QUI N’EST PAS FAIT, ET QUI EST SU : le biome ne commande PAS encore la génération des salles. Un module dit sa matière et sa mécanique, le générateur ne les lit pas — c’est le chantier suivant, et il touche au générateur, qu’il vaut mieux ne pas remuer dans le même lot. La longueur du plan, elle, gagne à être un multiple de six : l’écran l’avoue quand elle ne l’est pas (« 13 salles réparties au plus juste sur 6 modules ») au lieu d’annoncer un chiffre rond qui serait faux.',
      'MESURÉ. 35 tests neufs (763 au total, tous verts), type-check à 0 erreur, build propre. Le paquet passe de 1 063,66 à 1 073,85 Ko bruts, de 337,50 à 341,29 Ko comprimés : +3,79 Ko pour l’écran, ses données et son dessin. Le plan a été rendu dans un Chromium réel — sur un banc statique reprenant la feuille de style et le SVG du jeu, le rendu WebGL du moteur faisant tomber l’onglet dans l’environnement d’analyse. LA ROBE attend l’œil du concepteur.',
    ],
  },
  {
  ...
  {
    date: '01/09/2026 16:25',
    title: 'Un rail est une PAROI — franchissable au plasma seul, et ça se voit',
    notes: [
      'LA DEMANDE : « que les rails se comportent comme des parois quand on est en eau et glace », en gardant le design actuel en vapeur, mais qu’il soit plus clair qu’ils ne se traversent pas. Puis, correction du concepteur : les parois ne doivent être traversables QUE PAR LE PLASMA — pas par la vapeur ordinaire.',
      'CE QUI CHANGE POUR LE JOUEUR. Tous les rails ont désormais un corps, pas seulement les conduits : l’eau et la glace butent sur toute l’épaisseur du tube, la vapeur ORDINAIRE bute sur son cœur, et seul le PLASMA — de la vapeur sur une ligne dont l’arc est engagé — passe. Un rail n’est donc plus un décalque qu’on traverse au hasard de son état : c’est une porte dont la clef est une énigme.',
      'LE CŒUR PLUS MINCE QUE LE TUBE, ET C’EST DÉLIBÉRÉ. La vapeur s’arrête à 0,9 fois le rayon d’ionisation, pas au rayon de capture : repoussée jusque-là, elle ne pourrait plus jamais allumer ce qu’elle veut franchir — c’est le verrou du lot précédent, qui avait rendu le conduit inatteignable en jeu. Il reste trois unités de marge, et l’expulsion recale la POSITION au lieu de ne pousser que la prédiction : pousser la seule prédiction pompait de la vitesse, et le nuage repartait à 37 unités par bond.',
      'LA SORTIE DE L’ÉTAT PLASMA, le point que le concepteur avait signalé. Quand l’arc lâche alors que le corps est DANS le tube, la paroi le remet dehors sans le catapulter : le déplacement est borné à 180 unités par seconde. Trois tests gardent ce moment — l’arc qui tombe, la condensation en plein tube, la vapeur qui redevient ordinaire.',
      'LA ROBE. En vapeur sans arc, le rail garde exactement l’aspect qu’il avait : une ligne de champ ouverte. Dès que le corps se condense, la même bande s’épaissit en matière, gagne deux arêtes vives, un biseau et une veine claire en son milieu — parce qu’à cet instant elle ARRÊTE. Les coudes se raccordent en ONGLET : le liséré, tracé segment par segment, se brisait à chaque virage (une encoche à l’intérieur, un trou à l’extérieur) ; il se calcule maintenant en une passe, à l’intersection des droites décalées. Les bouts se referment en DEMI-LUNE, centrée sur la direction de sortie — un cercle entier y posait un anneau en travers de la paroi, une perle qui n’existait dans aucune collision.',
      'ET LA BASCULE CRÉPITE, DANS LES DEUX SENS. Quand une paroi s’ouvre au plasma ou se referme quand l’arc lâche, elle ne se contente pas de fondre : trois filaments serpentent le long de chaque tronçon, re-tirés trente fois par seconde, sur un cœur blanc qui court toute la ligne, et un éclat plus vif à chaque coude — c’est là que le champ force le plus. Une seule formule sert aux deux sens.',
      'HUIT DÉFAUTS TROUVÉS EN REVUE avant la mise en ligne, tous corrigés ici. LE PLUS GRAVE : deux tubes qui se croisent se départageaient sur le seul arc, et le rang du raccourci se perdait — un rail ORDINAIRE engagé, passant plus près, effaçait le laissez-passer du décor. Mesuré sur un conduit doublé d’un rail à 10 unités : 34 traversées tombaient à 20. Le classement est désormais explicite (raccourci ouvert > rail à l’arc > tube éteint), la distance ne départage qu’à rang égal — 34 traversées sur 34, et un test qui tombe sans le correctif.',
      'LE DÉPART POUVAIT NAÎTRE DANS UNE PAROI. Le générateur posait le corps sans regarder les rails, ce qui était sans conséquence tant qu’un rail n’avait pas de corps. Mesuré sur 1 500 graines : 463 tableaux à rail, 8 départs dans le tube (graines 78, 411, 549, 928, 1045, 1201, 1352, 1375), distance minimale ZÉRO. La vérification de tableau refuse maintenant ce départ ; après correctif, 459 tableaux à rail, 0 mauvais départ, distance minimale exactement 30. Un test de trois cents graines le grave (la graine 78 le déclenche).',
      'ET LA PAROI SE PEINT À L’ÉPAISSEUR QUI ARRÊTE. Le mur d’un conduit se dessinait à 75 unités devant un corps gazeux qui s’immobilise à 27 : « le tube qui ment sur sa taille », à l’envers cette fois. La largeur peinte suit maintenant l’état du corps. L’ÉCLAIR, lui, naissait de la POSITION de la bascule : un corps arrêté à 95 % de vapeur faisait crépiter chaque rail engagé POUR TOUJOURS. Il naît désormais du MOUVEMENT et retombe seul. Et la bascule se remet à zéro au changement de tableau — sans quoi le rail n° 0 du tableau suivant héritait de la valeur du précédent, et chaque chargement s’ouvrait sur une gerbe d’éclairs.',
      'UNE GÂCHETTE POUR PRÉVISUALISER, au passage : pousser une branche sur « previsu-go » déploie un APERÇU de ce qu’on y pousse, jamais la production. Le bouton « Run workflow » de GitHub se replie selon l’appareil et la largeur de la fenêtre — il était introuvable sur le poste du concepteur. La gâchette, elle, marche toujours. Et PUBLIER EST DÉSORMAIS L’EXCEPTION : le déploiement ne met en ligne que sur une poussée vers prod ou un lancement manuel ciblant la production, si bien qu’une branche déclenchante ajoutée par mégarde ne peut plus publier.',
      'MESURÉ. 744 tests verts (67 fichiers), contre 742 sur l’intégration : type-check à 0 erreur, construction propre. Les deux correctifs de fond sont vérifiés DANS LES DEUX SENS — retirés, leur test tombe ; remis, il passe.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LA DEMANDE : un écran pour gérer le déroulement de la run, paramétrer l’algorithme de choix des niveaux, et des outils pour jouer avec tout cela.',
      'CE QU’IL Y AVAIT AVANT, et c’était le vrai problème. Le plan de la voie se réglait dans un TIROIR du cahier des règles : deux crans (longueur, difficulté max) et trois cases. Tout le reste du déroulement — la FORME de la rampe, la POSTURE des rangs, la part des FIGURES — était écrit en dur dans voie.ts, et les QUATRE POIDS de l’algorithme de pioche étaient des constantes de compilation dans poule.ts. Pour voir l’effet d’un chiffre, il fallait DESCENDRE : un quart d’heure par essai, et le chiffre suivant, un autre quart d’heure.',
      'L’ÉCRAN « LA DESCENTE » (accueil, mode concepteur) fait trois choses, dans cet ordre. RÉGLER : le plan, la rampe (recul du sommet, respiration, finale), la posture (rangs sans danger, cadence du labyrinthe, cadence du contraste, figures au début et ensuite), et les quatre poids de la pioche (moment, difficulté, malus de mécanique répétée, coût d’un tableau muet). VOIR : la rampe dessinée barre par barre, et la TABLE du plan déroulé — une ligne par rang, moment, difficulté, ampleur, figures, posture — refaite à chaque cran, donc juste à l’instant où le chiffre change. ÉPROUVER : les outils ci-dessous.',
      'RIEN N’EST GÉNÉRÉ DANS L’ÉCRAN, et c’est un choix, pas une économie. Construire une salle coûte des dizaines de millisecondes et peut échouer sur une graine ingrate : un aperçu qui générerait figerait l’écran et mentirait sur les échecs. On ne déroule que les DÉCISIONS — moment, difficulté, posture, mécaniques tirées, tableau pioché — c’est-à-dire exactement ce que les réglages commandent. L’aperçu est donc instantané, et honnête.',
      'LE TIRAGE À BLANC déroule la descente entière et dit, rang par rang, quel tableau du pool la pioche choisirait. Il se REJOUE : la graine est à l’écran et se recopie — un réglage ne se juge qu’en ne changeant qu’une seule chose à la fois, donc à hasard égal. LE BILAN tire N descentes (100 par défaut, mesuré à 29 ms sur la bibliothèque livrée) et compte : combien de tableaux distincts sortent, à quelle fréquence, la répartition des mécaniques — et surtout la liste de ceux qu’AUCUNE descente ne propose. LE SAUT DE RANG pose la profondeur de la descente en cours où l’on veut : régler la fin d’une voie ne demande plus d’en jouer douze salles. Deux cases décident enfin des MÉMOIRES supposées tissées : décochées, on voit la descente d’un joueur qui n’a pas encore ses liens — là où les pools se vident.',
      'L’OUTIL A PARLÉ TOUT DE SUITE, et ce n’est pas flatteur. Bilan de 100 descentes sur la bibliothèque livrée : 12 tableaux proposés, 12 JAMAIS. La cause est connue et documentée dans poule.ts — aucun tableau livré ne porte encore la nomenclature « 21XX-MMD », et les tableaux muets gardent l’ordre de la bibliothèque au lieu d’être battus au hasard. La descente prend donc les douze premiers, toujours les mêmes, dans le même ordre ; la seconde moitié de la bibliothèque n’existe pour personne. RIEN N’EST CORRIGÉ ICI (coder les tableaux est un travail de nomenclature, pas de moteur) — mais ce lot le rend VISIBLE, et c’était l’objet.',
      'RIEN NE CHANGE POUR LE JOUEUR. Les huit réglages neufs et les quatre poids ont pour défauts les constantes qu’ils remplacent, AU CHIFFRE PRÈS, et un plan enregistré avant eux les retrouve tous : un test relit un plan à cinq champs (le format d’hier) et vérifie que la rampe qu’il décrit est identique à celle d’avant. Sans ce garde-fou, la descente d’un poste aurait changé de forme sans que personne n’ait rien réglé — le pire des défauts, celui qu’on n’impute à rien.',
      'DEUX GARDE-FOUS DE PLUS, parce que ce sont des réglages et non des constantes. Le coût d’un tableau MUET ne peut pas descendre sous le pire écart réel (2 moments + 9 crans de difficulté) : au-dessous, un tableau sans code passerait devant un tableau codé et la nomenclature ne servirait plus à rien. Et le contraste ne se pose jamais au DERNIER rang, quelle que soit la cadence réglée : la sortie se voit.',
      'UNE SEULE PORTE. Le panneau du cahier des règles est remplacé par un bouton vers l’écran, et ses styles sont partis avec lui — deux panneaux qui écrivent le même plan finissent par se contredire à l’écran, et du CSS que plus rien ne sélectionne est du poids qu’on télécharge pour rien. Un test garde la porte : les identifiants de la coque doivent exister dans index.html, et le bouton d’accueil rester un outil de concepteur.',
      'MESURÉ. 22 tests neufs (728 au total, tous verts), type-check à 0 erreur, build propre. Le paquet passe de 1 046,27 à 1 059,78 Ko bruts, de 331,23 à 336,17 Ko comprimés : +4,94 Ko pour l’écran, ses outils et le module d’aperçu. Écran ouvert dans un Chromium réel : la table, la rampe, le tirage, le bilan et le retour aux défauts vérifiés, aucune erreur de console. LA ROBE, elle, attend l’œil du concepteur.',
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      'LE SYMPTÔME, rapporté : « il n’y a pas d’ombres pour les couloirs et les chambres ». Dans un tableau bâti au kit de coque, les murs ne coupaient pas la lumière — la lampe d’une salle éclairait la salle voisine À TRAVERS la cloison, et un couloir restait aussi clair que la pièce qu’il quitte.',
      'LA CAUSE. Le cuiseur de carte de lumière donne à TOUT obstacle la même hauteur, 140 unités, et arrête le rayon dès qu’il passe par-dessus. C’est juste pour du mobilier : un bloc n’enferme pas la lumière. Mais une coque de chambre ou de couloir est une CLOISON, et elle était enjambée comme une caisse. MESURÉ sur deux chambres mitoyennes de 1200 unités (mur de 40, lampe à 600) : l’ombre du mur mitoyen s’arrêtait 196 unités derrière lui — 16 % de la pièce — et les 84 % restants baignaient en plein jour.',
      'LE REMÈDE : la hauteur devient une propriété de la BOÎTE, plus de la scène. Le rayon connaît son altitude à chaque pas et écarte tout obstacle déjà passé dessous — 140 unités pour le mobilier, jusqu’au plafond pour une coque, au-dessus de la lampe la plus haute que le moteur accepte. Les tamis (évent, éponge, vitre) restent bornés à la hauteur des blocs : sans cette séparation, les barreaux d’un évent auraient couché leur damier sur toute la salle.',
      'CE QUE LE CORRECTIF A RÉVÉLÉ. Le module d’accueil s’est assombri d’un coup à l’est : le sas de lancement à 0,33 d’éclairement, la passerelle à 0,26, le carrefour et son étal amputés d’un tiers. Le moteur n’allume que QUATRE lampes par tableau, et le hub en posait SIX — les deux éteintes étaient justement celles de l’est. Tout ce côté vivait de la lumière qui fuyait par-dessus les murs de l’ouest. Fermer les murs n’a pas éteint l’est : ça a montré qu’il n’y avait jamais eu de lampe.',
      'QUATRE LAMPES, TOUTES ALLUMÉES. Les deux de l’ouest deviennent des BANDEAUX de 1600 unités — un bandeau éclaire depuis le point de son segment le plus proche, donc un seul couvre chambre-couloir-chambre. Le premier va de la cuve à l’aile des endormis, le second du bac d’essai au centre de contrôle ; l’étal et le sas gardent leur plafonnier rond et leur teinte. MESURÉ coque par coque, sur les dix-sept : la moins éclairée passe de 0,255 à 0,595, la moyenne de 0,645 à 0,877, et aucune salle de l’ouest ne perd plus de deux centièmes.',
      'TROIS PLANS CONCURRENTS ONT ÉTÉ MESURÉS avant celui-là. Garder quatre plafonniers ronds parmi les six posés plafonne à 0,572 et laisse deux chambres de la route principale à 0,61. Poser les lampes DANS les couloirs, pour servir deux salles par une porte, tombe à 0,60 et vide les grandes chambres. Relever les intensités d’un quart lève bien les creux, mais écrête les salles déjà à 0,99.',
      'UNE TENTATIVE RETIRÉE, ET C’EST LA LEÇON DU LOT. J’ai cru qu’une porte fermée devait, elle aussi, monter au plafond — sinon la lumière passerait par-dessus son battant. Deux fois faux. Le rendu ne reçoit pas la liste de boîtes du solveur mais celle du tableau : une porte n’entre JAMAIS dans la carte de lumière, à aucune hauteur, et le drapeau ajouté était du code mort. Et sur le fond, une porte close se dessine en BARRIÈRE D’ÉNERGIE translucide — un champ qu’on voit au travers n’a pas à coucher l’ombre d’un mur. Les chiffres annoncés pour ce correctif venaient d’une mesure qui ajoutait les portes aux boîtes, ce que le moteur ne fait pas : ils ne décrivaient rien. Tout est revenu en arrière, et un commentaire dit désormais d’où vient la liste que le rendu voit vraiment.',
      'DEUX DÉFAUTS DE PLUS, TROUVÉS EN REVUE AVANT LA MISE EN LIGNE. La marche du rayon allait jusqu’à la lampe même sur un tableau SANS coque, où plus rien ne compte passé 140 unités : elle se borne maintenant au plus haut obstacle RÉELLEMENT posé — 16 % de pas en moins sur six tableaux en cuve, 43 848 rayons mesurés, pour un écart d’éclairement de 2,6 dix-millièmes au pire. Et la clé qui décide de recuire la carte oubliait la FORME du luminaire : basculer une lampe en bandeau sans la déplacer laissait les ombres d’avant à l’écran. Le défaut dormait tant qu’aucun tableau ne posait de bandeau ; le hub vient d’en poser deux.',
      'CE QUI N’EST PAS CORRIGÉ, ET QUI EST SU : la marche garde son plafond de 40 pas. Sur le hub, 2,80 % des rayons s’y arrêtent sans avoir atteint ni la lampe ni le noir — pour ceux-là, un mur lointain n’ombre pas. Relever le plafond coûterait à chaque texel de chaque tableau.',
      '683 tests verts, type-check à 0 erreur, build propre.',
    ],
  },
  {
