...
export function dessinMiniCarteSVG(
  ...
  const NOMS_HALTE: Record<string, string> = {
    ...
    minijeu: 'un mini-jeu — le couperet (laisser dépasser du trait ce que la lame doit trancher), le palet (glisser en glace jusqu’au centre), les rafales (traverser entre deux souffles), les orbites (lancé autour de trois puits de gravité), les cibles (des éclats de glace sur des mires) ou le métronome (pomper en cadence dans un puits)',
  }
