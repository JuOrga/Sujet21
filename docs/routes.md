...
| **Mini-jeu : les orbites** | nœud de la mini-carte, tiré à la graine avec les trois autres | trois **puits de gravité** en quinconce (`docs/puits.md`), le corps naît lancé (angle −50°, 310 u/s) et la gravité le porte : un virage autour de chaque puits, puis le croissant hydrophile où il se colle. Trois anneaux à passer dans l'ordre ; à chaque seconde, laisser porter (gratuit) ou corriger d'une éjection (chaque goutte coûte, la ligne se courbe à vue). Verdict : la part du volume gardée (≥ 90 / 70 / 45 %), un palier de moins par anneau manqué, rien sans le croissant. Le lancer, les anneaux et la cible viennent d'une recherche (`orbites.recherche.spec.ts`), deux tests de garde les tiennent. S'essaie sans run : « Jouer les orbites (essai) », `__orbites()` ; **P** écrit la prévision exacte. La ligne pointillée, elle, ne vit que dans l'éditeur | `REGLES_ORBITES` (`minijeux.ts`), la loi dans `puits.ts` |
