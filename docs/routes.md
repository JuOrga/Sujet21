...
| **Mini-jeu : le souffle** | nœud de la mini-carte, tiré à la graine avec les trois autres | un slalom en vapeur : la salle impose la vapeur, quatre grilles en quinconce laissent chacune un passage à un bout, la vapeur les traverse en s'y rognant. Les dashs sont sans compte (`dashBudget` 99) mais chacun chasse 7 % du nuage, et le nuage qui attend s'évapore six fois plus vite qu'en jeu (`REGLAGES_SOUFFLE`) : traverser coûte du nuage, contourner coûte du temps. Ce qui compte est le temps jusqu'à l'arrivée : ≤ 12 s en un souffle (mémoire ×3), ≤ 18 s vif (×1), ≤ 26 s essoufflé (×½), sinon dissipé. S'essaie sans run : « Jouer le souffle (essai) », `__souffle()` | `REGLES_SOUFFLE`, `REGLAGES_SOUFFLE` (`minijeux.ts`) |
...
| **Le souffle** — *fait le 17/09* | en vapeur, enchaîner les dashs dans un slalom de grilles ; traverser rogne le nuage, contourner prend du temps, et le temps s'évapore | le temps jusqu'à l'arrivée | même barème, par palier de temps |
