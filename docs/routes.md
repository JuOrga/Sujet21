...
| **Mini-jeu : les orbites** | nœud de la mini-carte, tiré à la graine avec les trois autres | trois **puits de gravité** en quinconce, cœur 450 et force 300 (`docs/puits.md` : aux défauts le corps arrivait en morceaux), le corps naît lancé (angle 36°, 260 u/s) et la gravité le porte : un virage autour de chaque puits, puis le croissant hydrophile où il se colle. Trois anneaux à passer dans l'ordre ; à chaque seconde, laisser porter (gratuit) ou corriger d'une éjection (chaque goutte coûte). Verdict : la part du volume gardée (≥ 90 / 70 / 45 %), un palier de moins par anneau manqué, rien sans le croissant. Le lancer, les anneaux et la cible viennent d'une recherche (`orbites.recherche.spec.ts`) que le vrai corps rejoue dans la vraie salle ; deux tests de garde les tiennent, dont le solveur sur toute la salle : le meilleur verdict sans un geste. S'essaie sans run : « Jouer les orbites (essai) », `__orbites()` ; **P** écrit la prévision exacte. La ligne pointillée, elle, ne vit que dans l'éditeur | `REGLES_ORBITES` (`minijeux.ts`), la loi dans `puits.ts` |
| **Mini-jeu : les cibles** | nœud de la mini-carte, tiré à la graine avec les quatre autres | le corps **en glace** tourne sans fin entre trois puits (la ronde de `docs/puits.md` §7, couchée) ; **le tir de glace** : viser ralentit le temps comme le dash de vapeur, relâcher détache un éclat (10 % du corps restant, 2 600 u/s à pleine puissance) qui file vers le doigt ; trois **mires** (10 · 5 · 10 points) sous des arcs (hydrophobes aux côtés, amorphe au milieu), un sol hydrophobe qui renvoie les ratés. Une touche vaut les points × la taille de l'éclat (plein pour le premier tir, jusqu'au double pour un amas), l'éclat disparaît, la mire reste ; le corps rétrécit à chaque tir, jamais sous trois grains ; 30 s ; verdict aux paliers 60 / 30 / 10. Le geste n'existe que par les réglages du tableau (`LevelDef.reglages`, le preset « ⚙ Tir de glace ») : ailleurs, en glace, rien ne part. S'essaie sans run : « Jouer les cibles (essai) », `__cibles()` | `REGLES_CIBLES`, `MIRES_CIBLES`, `REGLAGES_CIBLES` (`minijeux.ts`) ; `lanceEclat`, `touchesMires` (`solver.ts`) ; `MireDef` (`level.ts`) |
| **Mini-jeu : le métronome** | nœud de la mini-carte, tiré à la graine avec les cinq autres | **un seul puits** (cœur 1 000 u, force 600 : période 8,1 s, `docs/puits.md` §9) et le corps lancé de son centre (116 u/s, 150 u d'amplitude) : il oscille de part et d'autre et **repasse au centre toutes les 4,05 s, quoi qu'on fasse** — l'isochronie du cœur harmonique, c'est le battement. Le jeu : **pomper**. Une poussée ajoute v · Δv d'énergie — tout au passage du centre (la **fenêtre**, 160 u, allumée quand le corps y est), rien aux extrémités. Trois **anneaux** concentriques à atteindre (300 / 450 / 600 u) ; 45 s ; verdict : la part du volume gardée (≥ 65 / 50 / 35 %, ce qui fait corps plus ce qui est en prêt dans le halo), un palier de moins par anneau manqué. Des **éponges** sur les quatre parois boivent ce qu'on éjecte (sans elles la cuve est un bol : tout retombait au centre à travers le corps). L'éjection est accordée au jeu (2 400 u/s, 24 gouttes/s : à 64, une poussée tenue suffisait sans cadence). S'essaie sans run : « Jouer le métronome (essai) », `__metronome()` ; **P** écrit la prévision exacte | `REGLES_METRONOME`, `REGLAGES_METRONOME` (`minijeux.ts`), la loi dans `puits.ts`, la garde du vrai corps dans `sim/metronome.spec.ts` |
...
| **Les orbites** — *fait le 17/09* | lancé autour de trois puits de gravité, laisser l'orbite porter ou corriger d'une éjection ; passer trois anneaux, finir dans le croissant | la part du volume gardée, un palier de moins par anneau manqué | même barème |
| **Les cibles** — *fait le 17/09* | en glace, lancé entre trois puits (la ronde couchée) ; viser ralentit le temps, relâcher détache un éclat de glace qui file vers le doigt ; toucher trois mires sous leurs arcs, au-dessus d'un sol hydrophobe qui renvoie | les points en 30 s : chaque touche vaut les points de la mire × la taille de l'éclat (le premier tir vaut plein, un amas jusqu'au double), une série de touches à moins de 2 s multiplie (×2, ×3 au plus) ; l'éclat disparaît, la mire reste ; le corps rétrécit à chaque tir sans jamais s'épuiser | paliers 60 / 30 / 10 points, même barème ; le geste vient du tableau : son preset (`glaceTir`, « ⚙ Tir de glace ») ou une carte imposée (« Éclateur », `LevelDef.cartes`) — et la carte se tire aussi en run, comme les autres |
| **Le métronome** — *fait le 19/09* | lancé du centre d'un seul puits, osciller de part et d'autre ; pousser au passage du centre, dans le sens de la marche, pour gonfler l'amplitude — le battement ne change jamais (l'isochronie du cœur) ; atteindre trois anneaux concentriques | la part du volume gardée (ce qui fait corps plus ce qui est en prêt), un palier de moins par anneau manqué ; le temps limite à 45 s | paliers 65 / 50 / 35 %, même barème ; mesuré sur le vrai corps : en cadence 69 % en 22 s, sans arrêt 57 % en 6 s |
