...
| **Mini-jeu : le flipper** | nœud de la mini-carte, tiré à la graine avec les deux autres | une table portrait : une zone de glace la couvre (le corps **est** la bille), un courant permanent la tire vers le bas (la gravité), trois bumpers ronds et deux slingshots hydrophobes marquent un point par choc (`bumperHits` du solveur), deux flippers sont des portes éventail scénarisées dont le front pousse la bille quand on presse de leur côté (à gauche du centre : le gauche), un trou entre les deux. Trois billes, les points s'additionnent ; après chaque bille perdue, relancer ou valider. Paliers : ≥ 24 la mémoire triple, ≥ 12 elle vaut, ≥ 4 la moitié. Réglages : bumpers à 1,3 de restitution, pichenette 420. S'essaie sans run : « Jouer le flipper (essai) », `__flipper()` | `REGLES_FLIPPER`, `REGLAGES_FLIPPER` (`minijeux.ts`) |
...
| **Le flipper** — *fait le 17/09* | être la bille : la table gèle, le courant tire vers le bas, on presse à gauche ou à droite pour les flippers | les chocs de bumper, trois billes additionnées | même barème, par palier de points |
